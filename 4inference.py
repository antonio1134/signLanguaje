    ...
    if face_results.multi_face_landmarks:
        for face_landmarks in face_results.multi_face_landmarks:
            ...
            for landmark in face_landmarks.landmark:
                face_data_aux.append(landmark.x)
                face_data_aux.append(landmark.y)
    ...
    if len(all_landmarks) > 0:
        try:
            ...
            if predicted_character:
                x1 = int(min(x_all) * W) - 10 if x_all else 0
                y1 = int(min(y_all) * H) - 10 if y_all else 0
                x2 = int(max(x_all) * W) + 10 if x_all else W
                y2 = int(max(y_all) * H) + 10 if y_all else H
